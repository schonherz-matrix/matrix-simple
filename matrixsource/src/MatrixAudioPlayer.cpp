...
// --- Get state --- //
MatrixAudioPlayer::eState MatrixAudioPlayer::getState() const {
    return state;
}
...
float MatrixAudioPlayer::getVolume() const {
    return volume;
}
...
bool MatrixAudioPlayer::load(const void* data, size_t size) {
    ...
    FMOD_RESULT result = 
            system->createSound(
                reinterpret_cast<const char*>(this->data.get()),
                FMOD_OPENMEMORY | FMOD_LOOP_OFF,
                &soundInfo,
                &sound);
    if (result != FMOD_OK) {
        return false;
