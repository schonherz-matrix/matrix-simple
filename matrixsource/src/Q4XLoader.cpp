...
bool Q4XLoader::load(std::string file) {
    ...
    if (!ReadCompressed(inputFile, qp4) ||
        !ReadCompressed(inputFile, qpr)) 
    {
    ...
    if (soundDataSize > 4) {
        ...
        uint32_t uSoundFileSize = (uint32_t)cSoundFileSize[0] << 24
                | (uint32_t)cSoundFileSize[1] << 16
                | (uint32_t)cSoundFileSize[2] << 8 
                | cSoundFileSize[3];
...
bool ReadCompressed(istream& is, vector<uint8_t>& output) {
    ...
    chunkSize = uint32_t(output[0] << 24) | uint32_t(output[1] << 16) | uint32_t(output[2] << 8) | output[3];
...
const void* Q4XLoader::getSoundData() const {
    return soundData.get();
}

size_t Q4XLoader::getSoundDataSize() const {
