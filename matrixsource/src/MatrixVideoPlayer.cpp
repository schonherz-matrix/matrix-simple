...
MatrixVideoPlayer::MatrixVideoPlayer() {
    state = EMPTY;
}
...
auto MatrixVideoPlayer::getState() const -> eState {
    return state;
}
...
void MatrixVideoPlayer::displayThreadFunc() {
    ...
    while (state == PAUSED || state == PLAYING) {
        ...
        cout << "delta     = " << targetTimeDelta.count() / 1000.0f << " ms" << endl;
        cout << "delta comp= " << deltaCompensation.count() / 1000.0f << " ms" << endl;

        // lock that mutex lel
        unique_lock<mutex> lk(mtx);

        bool isExtraTask = cv.wait_for(lk, waitTime, [this] { return controlTaskQueue.size() > 0; });
        if (isExtraTask) {
            ...
            // if the task returns false, interrupt this frame and immediately start new
