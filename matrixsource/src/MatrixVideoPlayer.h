...
public:
    ...
    std::chrono::microseconds getDuration() const {return state == EMPTY ? std::chrono::microseconds(0) : frameTime*(intptr_t)frames.size();}
...
private:
    ...
    std::chrono::microseconds targetTimeDelta; // how much time is stream off from external time source
...
void MatrixVideoPlayer::setTime(std::chrono::duration<Rep, Period> time) {
    if (state == PLAYING || state == PAUSED) {
        ...
        controlTaskQueue.push(
                [this, time](std::chrono::microseconds frameElapsed) {
                    // compute required frame index and align playtime with next frame
                    std::chrono::microseconds timeDesired = std::chrono::duration_cast<std::chrono::microseconds>(time);
                    size_t numFrames = size_t(timeDesired.count()/frameTime.count());
                    std::chrono:: microseconds timeOvershoot = timeDesired - numFrames*frameTime;
...
template<class Rep, class Period>
void MatrixVideoPlayer::syncToExternalSource(std::chrono::duration<Rep, Period> externalTime) {
    if (state == PLAYING || state == PAUSED) {
        // acquire the mutex, then put the stub into the queue
        std::lock_guard<std::mutex> lk(mtx);
        controlTaskQueue.push(
