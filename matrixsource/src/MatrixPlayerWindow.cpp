...
public:
    PlayListItem(QListWidget * parent = nullptr, int type = Type) : QListWidgetItem(parent, type) {        
    }

    PlayListItem(const QString & text, QListWidget * parent = 0, int type = Type) : QListWidgetItem(parent, type) {
        splitAbsolutePath(text, path, fileName);
        QListWidgetItem::setText(fileName);
    }

    PlayListItem(const QIcon & icon, const QString & text, QListWidget * parent = nullptr, int type = Type) : QListWidgetItem(icon, "", parent, type){
        splitAbsolutePath(text, path, fileName);
        QListWidgetItem::setText(fileName);
    }
    ...
    virtual bool isBreakpoint() const {
        return false;
    }

protected:
    void splitAbsolutePath(const QString& fullName, QString& path, QString& fileName) {
...
public:
    PlayListBreakpoint(QListWidget* parent = nullptr, int type = Type) : PlayListItem(parent, type) {
    ...
    bool isBreakpoint() const override {
        return true;
    }
};
...
void MatrixPlayerWindow::on_buttonAddMedia_clicked() {
    QStringList files = QFileDialog::getOpenFileNames(this, "Add media", QString(), "*.q4x");
    for (auto it = files.begin(); it != files.end(); ++it) {
        QString fileName = *it;
        ui->playlistView->insertItem(ui->playlistView->currentRow()+1, new PlayListItem(fileName));
...
void MatrixPlayerWindow::on_buttonInsertBreakpoint_clicked() {
    ui->playlistView->insertItem(ui->playlistView->currentRow()+1, new PlayListBreakpoint());
}
    ...
    for (auto it = selectedMedia.begin(); it != selectedMedia.end(); ++it) {
        ...
        if (item == currentMedia) {
            ...
            if (numMedia > 0 && index >= 0) {
                ...
                currentMedia = dynamic_cast<PlayListItem*>(ui->playlistView->item(nextIndex));
            }
            else {
                currentMedia = nullptr;
            }
...
void MatrixPlayerWindow::on_playlistView_itemDoubleClicked(QListWidgetItem *item) {
    lock_guard<recursive_mutex> lk(matrixPlayerMutex);
...
void MatrixPlayerWindow::on_buttonPrevTrack_clicked() {
     seekPlaylist(-1);
}

void MatrixPlayerWindow::on_buttonNextTrack_clicked() {
    seekPlaylist(1);
}

void MatrixPlayerWindow::on_trackEnded() {
    bool isBreakpointHit = seekPlaylist(1);
    if ((ui->playlistView->count() > 0 && ui->playlistView->item(0) == currentMedia)
            || !autoplay
            || isBreakpointHit)
    {
        ui->buttonPlay->setText("\u25B6");
...
bool MatrixPlayerWindow::seekPlaylist(intptr_t offset) {
    if (currentMedia) {
        ...
        do {
            ...
            nextMedia = dynamic_cast<PlayListItem*>(ui->playlistView->item(nextIndex));
...
bool MatrixPlayerWindow::loadCurrentMedia() {
    if (currentMedia) {
        ...
        if (isLoaded) {
            ...
            QString durationText = "(" + secondsToTimestamp(durationUs/1000000) + ") ";            
            ui->labelTrackName->setText(durationText + dynamic_cast<PlayListItem*>(currentMedia)->text());
            ui->mediaTimeIndicator->setMaximum(durationUs/1000+1); // time indicator in ms!!!
        }
        else {
...
void PlayerListener::onTrackEnded() {   
    ...
    static const int dummy = []{ qRegisterMetaType<intptr_t>("intptr_t"); return 0; }();
    // play next or stop on first
    bool isInvoke = QMetaObject::invokeMethod(
                &parent, 
                "on_trackEnded", 
                Qt::QueuedConnection
                );
...
void MatrixPlayerWindow::displayFrame(const PlayerFrame& frame) {
    ...
    scale = std::min(
                (float)frameWidth / (float)width,
                (float)frameHeight / (float)height
                );
    ui->frameView->setTransform(QTransform(
        scale, 0, 0,
        0, scale, 0,
        0, 0, 1                                    
        ));
